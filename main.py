...
def _render_stage_review():
    ...
    st.markdown(
        ...
        '🔄 <strong>Vuoi modificare questo esercizio?</strong> '
    ...
    with col_nxt:
        if st.button("Successivo →", use_container_width=True, disabled=idx >= n_blocks - 1):
            st.session_state.review_sel_idx = idx + 1; st.rerun()
...
def _render_stage_final():
    ...
    with col_dl:
        ...
        st.markdown(
            ...
            'Ogni variante viene creata dall\'AI a partire dalla Fila A.</div>',
            unsafe_allow_html=True
        )

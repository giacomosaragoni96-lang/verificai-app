...
def _render_le_tue_verifiche():
    ...
    if verifiche_filtrate:
        for i, verifica in enumerate(verifiche_filtrate):
            ...
            # Container principale per ogni verifica
            st.markdown(f'''
            <div style="
                background: white;
                border: 2px solid #e5e7eb;
            ...
            " onmouseover="this.style.boxShadow='0 8px 24px rgba(0, 0, 0, 0.12)'; this.style.borderColor='#d1d5db';" 
               onmouseout="this.style.boxShadow='0 4px 12px rgba(0, 0, 0, 0.08)'; this.style.borderColor='#e5e7eb';">
                ...
                ">
                    <div style="display: flex; justify-content: space-between; align-items: start;">
                        <div style="flex: 1;">
                            <h3 style="font-size: 1.5rem; font-weight: 700; color: #1f2937; margin: 0 0 0.75rem 0; line-height: 1.3;">
                                {verifica.get('argomento', 'Verifica senza titolo')}
                            </h3>
                            <div style="display: flex; gap: 0.75rem; flex-wrap: wrap;">
                                <span style="background: #eff6ff; color: #1d4ed8; padding: 0.4rem 1rem; border-radius: 24px; font-size: 0.85rem; font-weight: 600; border: 1px solid #dbeafe;">
                                    📚 {verifica.get('materia', 'Sconosciuta')}
                                </span>
                                <span style="background: #f0fdf4; color: #059669; padding: 0.4rem 1rem; border-radius: 24px; font-size: 0.85rem; font-weight: 600; border: 1px solid #dcfce7;">
                                    🏫 {verifica.get('scuola', 'Non specificata')}
                                </span>
                                <span style="background: #fefce8; color: #ca8a04; padding: 0.4rem 1rem; border-radius: 24px; font-size: 0.85rem; font-weight: 600; border: 1px solid #fef3c7;">
                                    📝 {verifica.get('num_esercizi', '?')} esercizi
                                </span>
                                <span style="background: #f3f4f6; color: #6b7280; padding: 0.4rem 1rem; border-radius: 24px; font-size: 0.85rem; font-weight: 600; border: 1px solid #e5e7eb;">
                                    📅 {data_formattata}
                                </span>
                            </div>
                        </div>
                            ...
                            try:
                                ...
                                if pdf_bytes:
                                    ...
                                    if images:
                                        ...
                                        # Mostra anteprima in un container più grande
                                        st.markdown('''
                                        <div style="
                                            border: 2px solid #e5e7eb;
                                            border-radius: 12px;
                                            ...
                                            margin: 1rem 0;
                                        ">
                                            <div style="font-size: 1rem; font-weight: 600; color: #374151; margin-bottom: 1rem;">
                                                📄 Anteprima Verifica
                                            </div>
                                        ''', unsafe_allow_html=True)
                                        for j, img in enumerate(images[:3]):  # Max 3 pagine
                                            st.image(img, caption=f"Pagina {j+1}", width=800, use_container_width=False)
                                        st.markdown('</div>', unsafe_allow_html=True)
                                    else:
                                        st.error("⚠️ Impossibile generare anteprima immagini")
                                else:

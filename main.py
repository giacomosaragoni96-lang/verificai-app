...
def _render_le_tue_verifiche():
    ...
    Versione semplificata e pulita per risolvere problemi di visualizzazione
    """
    ...
    with col4:
        st.metric("Qualità ⭐", stats['qualita_media'])
    ...
    # Mostra verifiche reali con design semplice e pulito
    if verifiche_filtrate:
        for i, verifica in enumerate(verifiche_filtrate):
            ...
            with st.container():
                ...
                ">
                    <h3 style="font-size: 1.5rem; font-weight: 700; color: #1f2937; margin: 0 0 1rem 0;">
                        {verifica.get('argomento', 'Verifica senza titolo')}
                    </h3>
                    <div style="display: flex; gap: 0.75rem; flex-wrap: wrap; margin-bottom: 1.5rem;">
                        <span style="background: #eff6ff; color: #1d4ed8; padding: 0.4rem 1rem; border-radius: 20px; font-size: 0.85rem; font-weight: 600;">
                            📚 {verifica.get('materia', 'Sconosciuta')}
                        </span>
                        <span style="background: #f0fdf4; color: #059669; padding: 0.4rem 1rem; border-radius: 20px; font-size: 0.85rem; font-weight: 600;">
                            🏫 {verifica.get('scuola', 'Non specificata')}
                        </span>
                        <span style="background: #fefce8; color: #ca8a04; padding: 0.4rem 1rem; border-radius: 20px; font-size: 0.85rem; font-weight: 600;">
                            📝 {verifica.get('num_esercizi', '?')} esercizi
                        </span>
                        <span style="background: #f3f4f6; color: #6b7280; padding: 0.4rem 1rem; border-radius: 20px; font-size: 0.85rem; font-weight: 600;">
                            📅 {data_formattata}
                        </span>
                    </div>
                </div>
                ...
                with col1:
                    if st.button(f"👁️ Anteprima", key=f"preview_{verifica.get('id')}_{i}", use_container_width=True):
                        if verifica.get("latex_a"):
                            ...
                            st.warning("⚠️ Contenuto LaTeX non disponibile")
                
                with col2:
                    pdf_disabled = not LATEX_AVAILABLE
                    if st.button(f"📄 Scarica PDF", key=f"download_{verifica.get('id')}_{i}", 
                               disabled=pdf_disabled, use_container_width=True):
                        if verifica.get("latex_a"):
                            try:
                                pdf_bytes, error = compila_pdf(verifica["latex_a"])
                                if pdf_bytes:
                                    filename = f"{verifica.get('argomento', 'verifica').replace(' ', '_')}_{data_formattata.replace('/', '')}.pdf"
                                    st.download_button(
                                        label="⬇️ Scarica",
                                        data=pdf_bytes,
                                        file_name=filename,
                ...
                with col3:
                    if st.button(f"🎲 Facsimile", key=f"facsimile_{verifica.get('id')}_{i}", use_container_width=True):
                        st.info("🎲 Funzione facsimile in sviluppo...")
                
                with col4:
                    if st.button(f"🗑️ Elimina", key=f"delete_{verifica.get('id')}_{i}", use_container_width=True):
                        try:
                            res = supabase_admin.table("verifiche_storico")\
                                .delete()\
                            ...
                            if res.data:
                                st.success("✅ Verifica eliminata!")
                                st.rerun()
                            else:
                                st.error("⚠️ Errore eliminazione")
                        except Exception as e:
                            st.error(f"⚠️ Errore eliminazione: {e}")
                
                st.markdown('<div style="border-bottom: 1px solid #e5e7eb; margin: 1rem 0;"></div>', unsafe_allow_html=True)
    else:
...
def _render_percorso_b_form():
    ...
    <div style="background:{T['hint_bg']};border:2px solid {T['hint_border']}; 
                border-radius:12px;padding:16px 20px;font-size:1rem;color:{T['hint_text']}; 
                margin:12px 0 20px 0;font-family:DM Sans,sans-serif;line-height:1.6;
...
<div class="undo-btn-marker" style="display:none;height:0;line-height:0"></div>''',
                    ...
                    if st.button(
                        ...
                        _undo_latex = aggiungi_spaziatura_grafici_tabelle(_undo_latex)
    ...
    with col_pdf:
        ...
        if imgs:
            ...
            # Forza refresh dell'immagine (st.image non supporta key)
    ...
    if quick_regen:
        ...
        with _action_ph.status(f"Generazione variante esercizio {idx+1}…", expanded=True) as _qr_st:
            try:
                ...
                _qr_latex = aggiungi_spaziatura_grafici_tabelle(_qr_latex)
    ...
    if cambia_tutto:
        ...
        with _action_ph.status(f"Generazione nuovo esercizio {idx+1}…", expanded=True) as _ct_st:
            try:
                ...
                _ct_latex = aggiungi_spaziatura_grafici_tabelle(_ct_latex)
    ...
    if mostra_punteggi and n_blocks > 0:
        with st.expander("⚖️ Ricalibra Punteggi", expanded=False):
            ...
            if st.button(
                ...
                _latex_rc = aggiungi_spaziatura_grafici_tabelle(_latex_rc)
    ...
    if confirm_pdf:
        with st.spinner("Generazione PDF finale in corso… potrebbe richiedere qualche secondo"):
            ...
            latex_final = aggiungi_spaziatura_grafici_tabelle(latex_final)
...
def _render_stage_final():
    ...
    if st.session_state.get("force_preview_refresh", False):
        force_update = True
        logger.info("FORCE REFRESH: flag force_preview_refresh rilevato (final page)")
        # Resetta il flag dopo l'uso
        st.session_state.force_preview_refresh = False
    ...
    if force_update:
        ...
        if vA_pdf:
            try:
                logger.info(f"FORZATURA AGGIORNAMENTO PREVIEW (FINAL PAGE) - PDF ts: {current_pdf_ts}, Last preview: {last_preview_ts}")
                new_imgs, _ = pdf_to_images_bytes(vA_pdf)
                st.session_state.preview_images = new_imgs or []
                st.session_state.last_preview_ts = current_pdf_ts
                logger.info(f"Preview images aggiornate FORZATAMENTE (final page) - {len(new_imgs) if new_imgs else 0} pagine")
            except Exception as e:
                logger.error(f"Errore aggiornamento preview forzato (final page): {e}")
        else:
            logger.warning("PDF ts presente ma nessun PDF disponibile per preview (final page)")
    
    # Vecchia logica mantenuta come fallback
    elif current_pdf_ts and current_pdf_ts > last_preview_ts:
        vA_pdf = vA.get("pdf")
        if vA_pdf:
            try:
        ...
        with st.expander("📄 Anteprima PDF", expanded=False):
            if imgs:
                ...
                for pi, img_b in enumerate(imgs[:3]):
                    with cols_prev[pi]:
                        # Forza refresh delle immagini (st.image non supporta key)
...
if st.session_state.get('utente') and st.session_state.utente.email in ADMIN_EMAILS:
    ...
    # Pulsante per test integrato nell'app principale
    if st.sidebar.button("🧪 Test Verifiche (Integrato)", use_container_width=True):
        st.session_state["admin_test_mode"] = True
        st.rerun()
    
    st.sidebar.markdown("---")
    st.sidebar.markdown("### 📋 Sistema Esterno")
    if st.sidebar.button("🧪 Admin System (Porta 8502)", use_container_width=True):
        st.info("🔧 Apri in una nuova scheda:")
        st.code("streamlit run admin_test_system.py --server.port 8502")
        st.sidebar.markdown(
...
# ── ADMIN TEST MODE INTEGRATO ─────────────────────────────────────────────

# Modalità test integrata nell'app principale
if st.session_state.get("admin_test_mode", False):
    render_admin_test_mode_integrated()

def render_admin_test_mode_integrated():
    """Interfaccia test integrata nell'app principale"""
    ...
    st.markdown(
        ...
        f'<div>'
        f'<div style="font-size:1.5rem;font-weight:700;color:{T["text"]};">🧪 Admin Test Mode</div>'
        f'<div style="font-size:0.9rem;color:{T["text2"]};">Test verifiche e valutazione integrati</div>'
        f'</div>'
        f'<div>'
        f'<button onclick="window.location.reload()" style="padding:0.5rem 1rem;background:{T["accent"]};color:white;border:none;border-radius:8px;cursor:pointer;">'
        f'🔄 Ricarica'
        f'</button>'
        f'</div>'
        f'</div>'
    ...
    # Pulsante per uscire dalla modalità test
    if st.button("🔙 Torna a VerificAI", use_container_width=True):
        st.session_state["admin_test_mode"] = False
        st.rerun()
    
    # Import funzioni test (se necessario)
    try:
        from admin_test_system import (
            simulate_test_execution, 
            simulate_verification_content,
            save_test_session,
            save_evaluation_result
        )
    except ImportError:
        st.error("⚠️ Modulo admin_test_system non trovato. Assicurati che il file esista.")
        return
    ...
    with st.expander("⚙️ Configurazione Test", expanded=True):
        ...
        with col1:
            materia = st.selectbox(
                ...
                key="admin_test_materia"
            )
            
            argomento = st.text_input(
                "Argomento:",
                value="Trigonometria",
                key="admin_test_argomento"
            )
        ...
        punti_totali = st.number_input(
            ...
            max_value=100,
            value=30,
            key="admin_test_punti"
        )
        
        # Modalità debug
        debug_mode = st.checkbox("🔧 Modalità Debug (1 sola verifica)", value=True, key="admin_debug_mode")
    ...
    with col2:
        if 'admin_test_params' in st.session_state and st.session_state.admin_test_params:
            if st.button(f"▶️ Esegui {'Tutti i' if num_verifiche > 1 else 'Il'} Test", type="primary", use_container_width=True):
                execute_admin_test_integrated()
    
    # Risultati test
    if 'admin_test_results' in st.session_state:
        render_admin_test_results_integrated()
    
    # Valutazione
    if 'admin_verify_to_evaluate' in st.session_state:
        render_admin_evaluation_integrated()

def execute_admin_test_integrated():
    """Esegue test integrato nell'app principale"""
    try:
        from admin_test_system import simulate_test_execution, save_test_session
        import sqlite3
    except ImportError:
        st.error("⚠️ Funzioni test non disponibili")
        return
...
def render_admin_test_results_integrated():
    """Mostra risultati test integrati"""
    ...
    for i, result in enumerate(results):
        # Determina colore in base all'esito
        if result['esito'] == 'PASS':
            bg_color = "#d4edda"
            border_color = "#28a745"
        elif result['esito'] == 'FAIL':
        ...
        st.markdown(
            ...
            f'padding:1rem;margin-bottom:0.5rem;border-radius:4px;">'
        ...
        if st.button(f"✅ Valuta Questa Verifica", key=f"admin_valuta_{i}"):
            st.session_state.admin_verify_to_evaluate = result
            st.rerun()

def render_admin_evaluation_integrated():
    """Valutazione integrata nell'app principale"""
    try:
        from admin_test_system import simulate_verification_content, save_evaluation_result
    except ImportError:
        st.error("⚠️ Funzioni valutazione non disponibili")
        return
    
    verify_result = st.session_state.admin_verify_to_evaluate
    
    st.markdown("---")
    st.subheader(f"📝 Valutazione Verifica: {verify_result['test_id']}")
...
"""
    ...
    with col1:
        ...
        approve_verify = st.radio(
            ...
            ["✅ SI", "❌ NO"],
            key="admin_approve_verify"
        )
        
        if approve_verify == "✅ SI":
            commenti_verify = st.text_area(
                "Commenti verifica:",
                key="admin_commenti_verify",
        ...
        if st.button("💾 Salva Valutazione", type="primary"):
            try:
                ...
                if approve_verify == "✅ SI":
                    ...
                    st.info("ℹ️ Verifica non approvata, non salvata in banca dati.")
    ...
    with col2:
        if st.button("⏭️ Prossima Verifica"):
            del st.session_state.admin_verify_to_evaluate
            # Carica prossima verifica se disponibile
            st.rerun()
    
    with col3:
        if st.button("🔙 Torna Risultati"):
            del st.session_state.admin_verify_to_evaluate
            st.rerun()
